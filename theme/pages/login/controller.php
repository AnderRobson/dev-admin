...
class LoginController extends Controller
{
    ...
    public function index(array $data = null): void
    {
        if (! empty($data)) {
            ...
            if (! $user || ! password_verify($password, $user->password)) {
                echo $this->ajaxResponse("message", [
                    "type" => "danger",
                    "message" => "E-mail ou senha informados n�o conferem!"
                ]);

                return;
            }

            /** Valida��o de rede-social */
    ...
    public function forget(array $data = null): void
    {
        if (! empty($data)) {
            ...
            if (empty($user)) {
                echo $this->ajaxResponse("message", [
                    "type" => "danger",
                    "message" => "O e-mail informado n�o est� cadastrado"
                ]);
            ...
            flash("success", "Enviamos um lik de recupera��o de senha para seu e-mail");
    ...
    public function resetPassword(array $data): void
    {
        if (empty($_SESSION["forget"]) || ! $user = (new UserModel())->findById($_SESSION["forget"])) {
            if ($data["password"] != $data["password_confirmation"]) {
                flash("danger", "N�o foi poss�vel recuperar, tente novamente");
                echo $this->ajaxResponse("redirect", [
                    "url" => url("forget")
                ]);
                return;
        ...
        if ($data["password"] != $data["password_confirmation"]) {
            echo $this->ajaxResponse("message", [
                "type" => "warning",
                "message" => "Voc� informou duas senhas diferentes"
            ]);
            return;
        }
    ...
    public function reset(array $data): void
    {
        ...
        $errorForget = "N�o foi poss�vel recuperar, tente novamente";
    ...
    /**
     *  Realizando login via autentica��o Facebook.
     *
     * @param array|null $data
     */
    public function facebook(): void
    {
        ...
        if (empty((int) $configure->id)) {
            flash("danger", "Login com o Facebook n�o configurado");
            redirect("login");
        }

        $facebook = new Facebook([
            "clientId" => $configure->value['clientId'],
            "clientSecret" => $configure->value['clientSecret'],
            "redirectUri" => url("facebook"),
            "graphApiVersion" => $configure->value['graphApiVersion']
        ]);
        ...
        if ($error) {
            flash("danger", "N�o foi poss�vel logar com o Facebook");
            redirect("login");
            return;
        }
        ...
        if (! empty($userByEmail)) {
            flash("warning", "Ol� {$facebookUser->getFirstName()}, fa�a login para conectar sua conta Facebook");
            redirect("login");
            return;
        }

        /** Registrar usu�rio via Facebook */
        flash(
            "warning",
            "Ol� {$facebookUser->getFirstName()}, <strong>se j� tem uma conta clique em <a title='Fazer Login' href='" . url("login") . "'>FAZER LOGIN</a></strong>, ou complete seu cadastro"
        );
        redirect("register");
        return;
    }
    ...
    /**
     *  Realizando login via autentica��o Google.
     *
     */
    public function google(): void
    {
        ...
        if (empty((int) $configure->id)) {
            flash("danger", "Login com o Google n�o configurado");
            redirect("login");
        }
        ...
        if ($error) {
            flash("danger", "N�o foi poss�vel logar com o Google");
            redirect("login");
            return;
        }
        ...
        if (! empty($userByEmail)) {
            flash("warning", "Ol� {$googleUser->getFirstName()}, fa�a login para conectar sua conta Google");
            redirect("login");
            return;
        }

        /** Registrar usu�rio via Google */
        flash(
            "warning",
            "Ol� {$googleUser->getFirstName()}, <strong>se j� tem uma conta clique em <a title='Fazer Login' href='" . url("login") . "'>FAZER LOGIN</a></strong>, ou complete seu cadastro"
        );
        redirect("register");
        return;
    }
